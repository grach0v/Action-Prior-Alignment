...
echo "$MODEL_PATH"
echo "seen"
CUDA_VISIBLE_DEVICES="$GPU" "$PYTHON_BIN" -m a2.evaluate.test_place \
  --use_rope --load_model --model_path "$MODEL_PATH" \
  --log_suffix "place-${LOG_SUFFIX}" \
  --log_root "$LOG_ROOT" \
...
echo "unseen"
CUDA_VISIBLE_DEVICES="$GPU" "$PYTHON_BIN" -m a2.evaluate.test_place \
  --unseen --use_rope --load_model --model_path "$MODEL_PATH" \
  --log_suffix "place-${LOG_SUFFIX}-unseen" \
  --log_root "$LOG_ROOT" \
