...
CUDA_VISIBLE_DEVICES=$gpu taskset -c 0 python a2/train/main.py --lr 1e-4 --use_rope --data_path "$data_path" --log_suffix "$log_suffix"
...
for attn in $efficient_attn_list; do
  CUDA_VISIBLE_DEVICES=$gpu taskset -c 0 python a2/train/main.py \
    --lr 1e-4 \
    --use_rope \
    --data_path "$data_path" \
    --efficient_attn "$attn" \
    --log_suffix "efficient_attention_${attn}"
done
