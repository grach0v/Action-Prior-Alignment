...
class Environment:
    ...
    def generate_lang_goal(self):
        ...
        if template_id == 0:
            id = np.random.choice(range(len(UNSEEN_LABEL)), 1)[0]
            keyword = UNSEEN_LABEL[id]
            self.target_obj_dir = [UNSEEN_LABEL_DIR_MAP[id]]
            self.target_obj_lst = self.target_obj_dir
        else:
            if template_id == 1:
                id = np.random.choice(range(len(UNSEEN_GENERAL_LABEL)), 1)[0]
                keyword = UNSEEN_GENERAL_LABEL[id]
                self.target_obj_dir = UNSEEN_KEYWORD_DIR_MAP[keyword]
            elif template_id == 2:
                id = np.random.choice(range(len(UNSEEN_COLOR_SHAPE)), 1)[0]
                keyword = UNSEEN_COLOR_SHAPE[id]
                self.target_obj_dir = UNSEEN_KEYWORD_DIR_MAP[keyword]
            elif template_id == 3:
                id = np.random.choice(range(len(UNSEEN_COLOR_SHAPE)), 1)[0]
                keyword = UNSEEN_COLOR_SHAPE[id]
                ...
                id = np.random.choice(range(len(UNSEEN_FUNCTION)), 1)[0]
                keyword = UNSEEN_FUNCTION[id]
                self.target_obj_dir = UNSEEN_KEYWORD_DIR_MAP[keyword]
            
            if len(self.target_obj_dir) > 3:
                batch = np.random.choice(range(len(self.target_obj_dir)), 3, replace=False) 
    ...
    def generate_place_lang_goal(self, obj_bbox_ids, obj_bbox_centers, obj_bbox_sizes, pp_file=False):
        self.lang_goal, ref_obj_ids, ref_obj_centers, ref_regions, valid_mask = utils.generate_unseen_place_inst(self.obj_labels, self.obj_dirs, obj_bbox_ids, obj_bbox_centers, obj_bbox_sizes)
    ...
    def add_objects_for_place(self, num_obj, workspace_limits, test_data=False):
        ...
        with open(self.case_dir + self.test_file_name, "a") as out_file:
            ...
            for object_idx in range(len(obj_mesh_ind)):
                ...
                if curr_dir_label in UNSEEN_LABEL_DIR_MAP:
                    curr_ind = UNSEEN_LABEL_DIR_MAP.index(curr_dir_label)
                    curr_obj_label = UNSEEN_LABEL[curr_ind] 
                    self.obj_labels[body_id].append(curr_obj_label)

                # for objects that are have general meaningful labels
                for key in UNSEEN_KEYWORD_DIR_MAP_PLACE.keys():
                    if curr_dir_label in UNSEEN_KEYWORD_DIR_MAP_PLACE[key]:
